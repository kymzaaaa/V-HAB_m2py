...
class Zeolite5ATable:
    def __init__(self, k_l=None):
        """
        Initialize the Zeolite5ATable class with default or provided values.
        """
        # Constants
        self.csNames = None
        self.mfTothParamter_a0 = np.array([9.875E-7, 1.106E-8, 0, 0]) / 1000  # Convert from [1/kPa] to [1/Pa]
        self.mfTothParamter_b0 = np.array([6.761E-08, 4.714E-10, 0, 0]) / 1000  # Convert from [1/kPa] to [1/Pa]
        self.mfTothParamter_E = np.array([5.625E3, 9.955E3, 0, 0])
        self.mfTothParamter_t0 = np.array([2.700E-1, 3.548E-1, 0, 0])
        self.mfTothParamter_c0 = np.array([-2.002E1, -5.114E1, 0, 0])
        self.fUnivGasConst_R = 8.314  # Universal gas constant [J/(mol*K)]
        self.fR_pellet = 1.05e-3  # Pellet radius [m]
        self.fPelletPorosity = 0.4  # Pellet porosity [-]
        self.afD_collision = [2.8, 3.7979, 3.7412, 3.4822]  # Collision diameter [Å]
        ...
        self.D_p = np.nan
        self.D_c = np.nan
        self.k_l = np.array([0.0007, 0, 0, 0]) if k_l is None else np.array(k_l)
        self.K = np.nan
    ...
    def calculate_q_equ(self, afC_in, fTemperature, fRhoSorbent, csNames, _):
        ...
        """
        p_i = afC_in * self.fUnivGasConst_R * fTemperature  # Partial pressures [Pa]
